...
!IF (JKK == 14 .AND. JK == 12 .AND. JL == 6) THEN
! PRINT *, " CUBASE 489 ", ZLGLAC
! PRINT *, " CUBASE 490 ", ZDQ
! PRINT *, " CUBASE 491 ", ZTU(JL,JK)
! PRINT *, " CUBASE 492 ", ZTU(JL,JK+1)
!ENDIF
...
IF (JKK == 14 .AND. JK == 12 .AND. JL == 6) THEN
 WRITE (*, '(" CUBASE 503 ",E30.20)') ZTU(JL,JK)
ENDIF
        ZTU(JL,JK)     = ZTU(JL,JK)+RALFDCP*ZLGLAC
IF (JKK == 14 .AND. JK == 12 .AND. JL == 6) THEN
 WRITE (*, '(" CUBASE 507 ",E30.20)') ZTU(JL,JK)
 WRITE (*, '(" CUBASE 508 ",E30.20)') RALFDCP
 WRITE (*, '(" CUBASE 509 ",E30.20)') ZLGLAC
ENDIF
   ...
   ! Buoyancy on half and full levels
        ...
        ZTVUH           = (1.0_JPRB+RETV*ZQU(JL,JK)-ZLU(JL,JK))*ZTU(JL,JK)&
         & +RALFDCP*ZLGLAC  
IF (JKK == 14 .AND. JK == 12 .AND. JL == 6) THEN
 WRITE (*, '(" CUBASE 510 ",E30.20)') ZQU(JL,JK)
 WRITE (*, '(" CUBASE 511 ",E30.20)') ZLU(JL,JK)
 WRITE (*, '(" CUBASE 512 ",E30.20)') ZTU(JL,JK)
 PRINT *, " CUBASE 513 ", ZTU(JL,JK)
 WRITE (*, '(" CUBASE 514 ",E30.20)') ZLGLAC
 WRITE (*, '(" CUBASE 515 ",E30.20)') ZTVUH
 WRITE (*, '(" CUBASE 516 ",2E30.20)') RETV, RALFDCP
ENDIF
        ZTVENH          = (1.0_JPRB+RETV*ZQENH(JL,JK)) &
         & *(ZSENH(JL,JK)-PGEOH(JL,JK))*ZRCPD  
IF (JKK == 14 .AND. JK == 12 .AND. JL == 6) THEN
  PRINT *, " CUBASE 521 ", ZTVUH
  PRINT *, " CUBASE 522 ", ZTVENH
  PRINT *, " CUBASE 523 ", RG
ENDIF
        ZBUOH(JL,JK)   = (ZTVUH-ZTVENH)*RG/ZTVENH
IF (JKK == 14 .AND. JK == 12 .AND. JL == 6) THEN
  PRINT *, " CUBASE 527 ", ZBUOH(JL,JK)
  PRINT *, " CUBASE 528 ", ZTVUH
  PRINT *, " CUBASE 529 ", ZTVENH
  PRINT *, " CUBASE 530 ", RG
ENDIF

        ZBUOF          = (ZBUOH(JL,JK) + ZBUOH(JL,JK+1))*0.5_JPRB
IF (JKK == 14 .AND. JK == 12 .AND. JL == 6) THEN
  PRINT *, " CUBASE 527 ", ZBUOH(JL,JK), ZBUOH(JL,JK+1)
ENDIF
...
IF (JKK == 14) PRINT *, " CUBASE 521 ", JK, ZCAPE (6,14)
        ZCAPE(JL,JKK)  = ZCAPE(JL,JKK) + MAX(0.0_JPRB,ZBUOF*ZDZ(JL))
IF (JKK == 14 .AND. JK == 12) THEN
  PRINT *, " CUBASE 545 ", ZBUOF, ZDZ (6)
  PRINT *, " CUBASE 546 ", ZCAPE (6,14)
ENDIF
