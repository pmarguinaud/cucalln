...
DO JKK=KLEV, JKT1,-1
  ...
  DO JK=JKK-1, JKT2,-1
    ...
    IF (JKK==14.AND.JK==12) THEN
      WRITE (*,'(" CUBASE 481 ",E30.20)') ZTU (JLON, JK)
    ENDIF

    CALL CUADJTQ_SINGLE_COLUMN (YDTHF, YDCST, YDEPHLI, KIDIA, KFDIA, KLON, KLEV, IK&
    &, ZPH, ZTU, ZQU, LLGO_ON, 1, LDEBUG=JKK==14.AND.JK==12, YDSTACK=YLSTACK)

    IF (JKK==14.AND.JK==12) THEN
      WRITE (*,'(" CUBASE 487 ",E30.20)') ZTU (JLON, JK)
    ENDIF
    ...
    IF (LLGO_ON (JLON)) THEN
      ...
      IF (JKK==14.AND.JK==12.AND.JLON==6) THEN
        WRITE (*,'(" CUBASE 503 ",E30.20)') ZTU (JLON, JK)
      ENDIF

      ZTU (JLON, JK)=ZTU (JLON, JK)+YDTHF%RALFDCP*ZLGLAC

      IF (JKK==14.AND.JK==12.AND.JLON==6) THEN
        WRITE (*,'(" CUBASE 507 ",E30.20)') ZTU (JLON, JK)
        WRITE (*,'(" CUBASE 508 ",E30.20)') YDTHF%RALFDCP
      ...
      IF (JKK==14.AND.JK==12.AND.JLON==6) THEN
        WRITE (*,'(" CUBASE 510 ",E30.20)') ZQU (JLON, JK)
        WRITE (*,'(" CUBASE 511 ",E30.20)') ZLU (JLON, JK)
        WRITE (*,'(" CUBASE 512 ",E30.20)') ZTU (JLON, JK)
        PRINT*," CUBASE 513 ", ZTU (JLON, JK)
        WRITE (*,'(" CUBASE 514 ",E30.20)') ZLGLAC
        WRITE (*,'(" CUBASE 515 ",E30.20)') ZTVUH
        WRITE (*,'(" CUBASE 516 ",2E30.20)') YDCST%RETV, YDTHF%RALFDCP
      ENDIF
      ...
      IF (JKK==14)  THEN
          PRINT*," CUBASE 521 ", JK, ZCAPE (JLON, 14)
      ENDIF

      ZCAPE (JLON, JKK)=ZCAPE (JLON, JKK)+MAX (0.0_JPRB, ZBUOF*ZDZ )

      IF (JKK==14.AND.JK==12) THEN
        PRINT*," CUBASE 545 ", ZBUOF, ZDZ 
        PRINT*," CUBASE 546 ", ZCAPE (JLON, 14)
      ENDIF
  ...
  IF (JKK==KLEV.OR.(JKK==KLEV-1.AND.KINDEX==KLEV-1)) THEN
    ...
    LLDEEP =.FALSE.
    LLGO_ON (JLON)=.NOT.LLDEEP 

    IF (KINDEX==KLEV-1)  THEN
        LLGO_ON (JLON)=.NOT.LLDCUM 
    ENDIF
...
PRINT*," CUBASE 697 ", ZCAPE (JLON, 14)

PCAPE (JLON)=MAXVAL (ZCAPE (JLON,:))

PRINT*," CUBASE 700 ", PCAPE (JLON)
