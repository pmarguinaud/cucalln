...
INTERFACE NPROMIZE
  PROCEDURE NPROMIZE2P, NPROMIZE3P, NPROMIZE4P, NPROMIZE2L
END INTERFACE

INTEGER (KIND=JPIM), PARAMETER :: ILUN = 77
INTEGER (KIND=JPIM) :: JBLK, JLON

INTEGER(KIND=JPIM)    :: NGPTOT, NPROMA, NGPBLKS
...
& KIDIA, KFDIA, NGPTOT, KSMAX, KLEV, PDX(:,JBLK), KSPPN2D, LDLAND(:,JBLK), LDSLPHY, PTSPHY,      &
...
LOGICAL,                  ALLOCATABLE :: LDLAND_A(:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PLCRIT_AER_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PTM1_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PQM1_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PUM1_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PVM1_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PCM1_A(:,:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PLITOT_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PVERVEL_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PQHFL_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PAHFS_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PAPHM1_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PAP_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PAPH_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PGEO_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PGEOH_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PTENT_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PTENQ_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PTENU_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PTENV_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PGAW_A(:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PCUCONVCA_A(:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PGP2DSPP_A(:,:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PDX_A(:,:)
REAL (KIND=JPRB),         ALLOCATABLE :: PTENC_A(:,:,:,:)
LOGICAL,                  ALLOCATABLE :: LDSHCV_A(:,:)
...
ALLOCATE (LDLAND_A(NGPTOT,1))
ALLOCATE (PLCRIT_AER_A(NGPTOT,KLEV,1))
ALLOCATE (PTM1_A(NGPTOT,KLEV,1))
ALLOCATE (PQM1_A(NGPTOT,KLEV,1))
ALLOCATE (PUM1_A(NGPTOT,KLEV,1))
ALLOCATE (PVM1_A(NGPTOT,KLEV,1))
ALLOCATE (PCM1_A(NGPTOT,KLEV,KTRAC,1))
ALLOCATE (PLITOT_A(NGPTOT,KLEV,1))
ALLOCATE (PVERVEL_A(NGPTOT,KLEV,1))
ALLOCATE (PQHFL_A(NGPTOT,KLEV+1,1))
ALLOCATE (PAHFS_A(NGPTOT,KLEV+1,1))
ALLOCATE (PAPHM1_A(NGPTOT,KLEV+1,1))
ALLOCATE (PAP_A(NGPTOT,KLEV,1))
ALLOCATE (PAPH_A(NGPTOT,KLEV+1,1))
ALLOCATE (PGEO_A(NGPTOT,KLEV,1))
ALLOCATE (PGEOH_A(NGPTOT,KLEV+1,1))
ALLOCATE (PTENT_A(NGPTOT,KLEV,1))
ALLOCATE (PTENQ_A(NGPTOT,KLEV,1))
ALLOCATE (PTENU_A(NGPTOT,KLEV,1))
ALLOCATE (PTENV_A(NGPTOT,KLEV,1))
ALLOCATE (PGAW_A(NGPTOT,1))
ALLOCATE (PCUCONVCA_A(NGPTOT,1))
ALLOCATE (PGP2DSPP_A(NGPTOT,KSPPN2D,1))
ALLOCATE (PDX_A(NGPTOT,1))
ALLOCATE (PTENC_A(NGPTOT,KLEV,KTRAC,1))
ALLOCATE (LDSHCV_A(NGPTOT,1))
...
ALLOCATE (PCM1(NPROMA,KLEV,KTRAC,NGPBLKS))
...
ALLOCATE (PTENC(NPROMA,KLEV,KTRAC,NGPBLKS))
...
READ (ILUN) LDLAND_A            ; CALL NPROMIZE (NPROMA, LDLAND_A          , LDLAND      )
READ (ILUN) PLCRIT_AER_A        ; CALL NPROMIZE (NPROMA, PLCRIT_AER_A      , PLCRIT_AER  )
READ (ILUN) PTM1_A              ; CALL NPROMIZE (NPROMA, PTM1_A            , PTM1        )
READ (ILUN) PQM1_A              ; CALL NPROMIZE (NPROMA, PQM1_A            , PQM1        )
READ (ILUN) PUM1_A              ; CALL NPROMIZE (NPROMA, PUM1_A            , PUM1        )
READ (ILUN) PVM1_A              ; CALL NPROMIZE (NPROMA, PVM1_A            , PVM1        )
READ (ILUN) PCM1_A              ; CALL NPROMIZE (NPROMA, PCM1_A            , PCM1        )
READ (ILUN) PLITOT_A            ; CALL NPROMIZE (NPROMA, PLITOT_A          , PLITOT      )
READ (ILUN) PVERVEL_A           ; CALL NPROMIZE (NPROMA, PVERVEL_A         , PVERVEL     )
READ (ILUN) PQHFL_A             ; CALL NPROMIZE (NPROMA, PQHFL_A           , PQHFL       )
READ (ILUN) PAHFS_A             ; CALL NPROMIZE (NPROMA, PAHFS_A           , PAHFS       )
READ (ILUN) PAPHM1_A            ; CALL NPROMIZE (NPROMA, PAPHM1_A          , PAPHM1      )
READ (ILUN) PAP_A               ; CALL NPROMIZE (NPROMA, PAP_A             , PAP         )
READ (ILUN) PAPH_A              ; CALL NPROMIZE (NPROMA, PAPH_A            , PAPH        )
READ (ILUN) PGEO_A              ; CALL NPROMIZE (NPROMA, PGEO_A            , PGEO        )
READ (ILUN) PGEOH_A             ; CALL NPROMIZE (NPROMA, PGEOH_A           , PGEOH       )
READ (ILUN) PTENT_A             ; CALL NPROMIZE (NPROMA, PTENT_A           , PTENT       )
READ (ILUN) PTENQ_A             ; CALL NPROMIZE (NPROMA, PTENQ_A           , PTENQ       )
READ (ILUN) PTENU_A             ; CALL NPROMIZE (NPROMA, PTENU_A           , PTENU       )
READ (ILUN) PTENV_A             ; CALL NPROMIZE (NPROMA, PTENV_A           , PTENV       )
READ (ILUN) PGAW_A              ; CALL NPROMIZE (NPROMA, PGAW_A            , PGAW        )
READ (ILUN) PCUCONVCA_A         ; CALL NPROMIZE (NPROMA, PCUCONVCA_A       , PCUCONVCA   )
READ (ILUN) PGP2DSPP_A          ; CALL NPROMIZE (NPROMA, PGP2DSPP_A        , PGP2DSPP    )
READ (ILUN) PDX_A               ; CALL NPROMIZE (NPROMA, PDX_A             , PDX         )
READ (ILUN) PTENC_A             ; CALL NPROMIZE (NPROMA, PTENC_A           , PTENC       )
READ (ILUN) LDSHCV_A            ; CALL NPROMIZE (NPROMA, LDSHCV_A          , LDSHCV      )
CLOSE (ILUN)

END SUBROUTINE


SUBROUTINE NPROMIZE2L (KPROMA, LDI, LDO)

INTEGER :: KPROMA
LOGICAL,  INTENT (IN)  :: LDI (:,:) 
LOGICAL,  INTENT (OUT) :: LDO (:,:)

INTEGER :: I, J, IGPBLK, IGPTOT, IGP, JLON, JIDIA, JFDIA, IBL

IF (SIZE (LDI, 2) /= 1) STOP 1
...
DO IGP = 1, IGPTOT, KPROMA
  ...
  DO JLON = JIDIA, JFDIA
    LDO (JLON, IBL) = LDI (IGP + (JLON - 1), 1)
  ENDDO

  DO JLON = JFDIA+1, KPROMA
    LDO (JLON, IBL) = LDO (JFDIA, IBL)
  ENDDO
...
SUBROUTINE NPROMIZE2P (KPROMA, PI, PO)
...
SUBROUTINE NPROMIZE3P (KPROMA, PI, PO)
...
SUBROUTINE NPROMIZE4P (KPROMA, PI, PO)
